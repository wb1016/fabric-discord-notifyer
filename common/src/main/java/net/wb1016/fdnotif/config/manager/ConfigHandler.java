...
import net.wb1016.fdnotif.config.MainConfig;
import net.wb1016.fdnotif.config.MessageConfig;
...
private static ConfigHolder parseConfig(JsonObject jsonObject) {
	...
	if (needUpgrade) {
		LOGGER.info("An old version of fdnotif config has been detected! Upgrading to latest.");
		}
		...
		for (JsonElement jsonElement : readList(ainObject.getAsJsonArray("chatChannels"))) {
		...
		for (JsonElement jsonElement : readList(mainObject.getAsJsonArray("logChannels"))) {
			if (jsonElement.isJsonPrimitive()) {
				JsonPrimitive jsonPrimitive = jsonElement.getAsJsonPrimitive();
		...
		for (JsonElement jsonElement : readList(mainObject.getAsJsonArray("emojiMap"))) {
